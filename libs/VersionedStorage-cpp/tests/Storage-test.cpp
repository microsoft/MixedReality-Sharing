...
class Storage_Test : public ::testing::Test {
 ...
 protected:
  enum class ApplyKind { Direct, Serialized };

  static constexpr std::array<ApplyKind, 2> kApplyKinds = {
      ApplyKind::Direct, ApplyKind::Serialized};
  ...
  Storage::TransactionResult ApplyTransaction(
      ...
      ApplyKind kind) {
    assert(kind == ApplyKind::Direct || kind == ApplyKind::Serialized);
    if (kind == ApplyKind::Direct) {
      return storage.ApplyTransaction(transaction_builder);
    }
    Serialization::BitstreamWriter bitstream_writer;
    std::vector<std::byte> byte_stream;
...
TEST_F(Storage_Test, unsatisfied_subkeys_count_prerequisite) {
  for (auto apply_kind : kApplyKinds) {
    auto storage = std::make_shared<Storage>(behavior_);
    auto transaction = TransactionBuilder::Create(behavior_);
    transaction->RequireSubkeysCount(MakeKeyDescriptor(7), 6);
    EXPECT_EQ(ApplyTransaction(*storage, *transaction, apply_kind),
...
TEST_F(Storage_Test, transaction_with_no_effect) {
  for (auto apply_kind : kApplyKinds) {
    ...
    ASSERT_EQ(ApplyTransaction(*storage, *transaction, apply_kind),
