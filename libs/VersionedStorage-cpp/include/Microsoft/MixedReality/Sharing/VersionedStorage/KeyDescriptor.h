...
// look for a string_view, it can implement KeyDescriptor that will work with
...
//
// Most KeyDescriptor objects should be created on the stack and discarded after
...
// mutate KeyDescriptor, rendering it unusable for any further calls.
class KeyDescriptor {
 public:
  ...
  // Returns a handle to the key that behaves the same way as this KeyDescriptor
  // object.
  // No other methods will be called after this one, so if this
  // KeyDescriptor owns the handle already, it can just transfer the
  ...
  // and this KeyDescriptor doesn't own a KeyHandle already, it may be cheaper
 ...
 protected:
  KeyDescriptor(uint64_t key_hash) : key_hash_(key_hash) {}
